/**
 * Renders an article card with image, metadata, heading, and description
 * @param {Object} card
 ...
 *
 * Note: Bylines are only rendered on blog hero sections, not on article cards.
 */
export function renderArticleCard(card) {
  const { href, image, tagText, tagMeta, heading, body, isMuted, ariaDisabled } = card;
  ...
  const innerHtml = `
    ...
    <div class="article-card-body">${tagHtml}
      ...
      <p class="paragraph-sm utility-text-secondary">${body}</p>${card.textButton ? `
